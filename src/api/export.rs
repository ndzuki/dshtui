...
//!   `session/page` 全量 records 重建（行格式 = REQ-008 live 锁定结论，见
//!   `model::export` 模块头：session header / record 解包 / chunkrow→*-chunks +
//!   seq0/time0 / 顺时排序；page 为聚合视图，不可还原官方逐 delta 行，
...
/// 循环 `session/page` 收集该会话全量 raw records（页内 seq 升序、跨页
/// beforeSeq 独占上界单调退旧、hasMore=false 停止），收集完成后按 seq 顺时
/// 排序，再逐行写 header + record JSON 到目标目录 tmp 文件，完成后原子
/// rename。
///
/// 返回 `(records 数, final_path)`。任何页错误/取消 → 删除 tmp、不留半成品。
/// 行格式 = REQ-008 live 锁定（AC-008-15，见 `model::export` 模块头）；本
/// 函数可测口径 = records 行数对账 + tmp+rename 落盘 + 取消清理。
    ...
    C: Fn() -> bool + Send,
{
    ...
    let run: Result<u64, ClientError> = async {
        // 顺序（REQ-008 live 锁定）：官方 0.1.2-rc.1 `session/page` 页内升序、
        // 跨页（beforeSeq 走旧）整体逆序——收集顺序不能直接作输出顺序。这里
        // 先全量收集 raw records（page 聚合视图，紧凑；单会话通常 <2k 条），
        // 收集完成后统一按 seq 顺时排序再逐行落盘（`rebuild_sort_chronological`
        // 单一实现，与 model 层 `rebuild_jsonl` seam 同规则）。仍是同目录
        // tmp+rename 原子：页错误/取消 → 删除 tmp、不留半成品。
        ...
        // 收集完成 → 顺时排序 + 逐行写（header 先行）。
        crate::model::export::rebuild_sort_chronological(&mut records);
        let mut bytes: u64 = 0;
